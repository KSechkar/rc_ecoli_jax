...
def main():
    ...
    x0s_with_fitted_parvec = jnp.concatenate(
        (x0s, jnp.multiply(np.ones((x0s.shape[0], len(fitted_parvec))), fitted_parvec)), axis=1)
    fitted_model_sol = vmapped_diffeqsolve_forx0s(x0s_with_fitted_parvec)
    fitted_predictions = get_l_phir_forxs(fitted_model_sol.ys)
    ...
    x0s_with_fitted_parvec = jnp.concatenate((x0s, jnp.multiply(np.ones((x0s.shape[0], len(fitted_parvec))), fitted_parvec)), axis=1)
    fitted_model_sol = vmapped_diffeqsolve_forx0s(x0s_with_fitted_parvec)
    model_predictions = get_l_phir_forxs(fitted_model_sol.ys)

    # initialise the plot
    bkplot.output_file('fit_eval_figures/model_vs_scott2010.html')
    ...
    for i in range(0, len(vl_nutr_quals)):
        ...
        vl_x0 = jnp.concatenate(
            (jnp.array(cellmodel_auxil.x0_from_init_conds(init_conds, circuit_genes, circuit_miscs)),
             fitted_parvec)
    ...
    bkplot.output_file('fit_eval_figures/model_vs_chure2023.html')
