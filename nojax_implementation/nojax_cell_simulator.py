## nojax_cell_simulator.py
# Python implementation of the cell simulator without JAX
...
from nojax_het_modules.nojax_aif_controller import initialise as aif_init, ode as aif_ode, F_calc as aif_F_calc, v as aif_v
...
class CellModelAuxiliary:
    ...
    # package synthetic gene parameters into vectors for calculating k values
    def synth_gene_params_vectorised(self, par,  # system parameters
        ...
        return (kplus_het, kminus_het, n_het, d_het)
    ...
    def plot_protein_masses(self, ts, xs,
        ...
        if (len(circuit_genes) != 0):
            ...
            top_line = bottom_line + np.sum(xs[:, 8 + len(circuit_genes):8 + len(circuit_genes) * 2] * np.array(
                self.synth_gene_params_vectorised(par, circuit_genes)[2], ndmin=2), axis=1)
    ...
    def plot_phys_variables(self, ts, xs,
        ...
        e, l, F_r, nu, _, T, D  = self.get_e_l_Fr_nu_psi_T_D(ts, xs, par, circuit_genes, circuit_miscs,
                                                                            circuit_name2pos)
    ...
    def get_e_l_Fr_nu_psi_T_D(self, t, x,
        ...
        # vector of Synthetic Gene Parameters
        sgpv = self.synth_gene_params_vectorised(par, circuit_genes)
        kplus_het, kminus_het, n_het, d_het = sgpv
        ...
        k_het = k_calc((np.atleast_2d(np.array(e) * np.ones((len(circuit_genes), 1)))).T,
                       np.atleast_2d(kplus_het) * np.ones((len(e), 1)),
                       np.atleast_2d(kminus_het) * np.ones((len(e), 1)),
                       np.atleast_2d(n_het) * np.ones((len(e), 1)), kcmh)  # heterologous genes
        ...
        m_het_div_k_het = np.sum(x[:, 8:8 + len(circuit_genes)] / k_het, axis=1)  # heterologous protein synthesis flux
        ...
        return e, l, F_r, nu, np.multiply(psi, l), T, D
    ...
    def plot_phys_variables_multiple(self, ts, xss,
        ...
        sources = {}
        for i in range(0, len(xss)):
            e, l, F_r, nu, psi, T, D = self.get_e_l_Fr_nu_psi_T_D(ts, xss[i, :, :],
...
def ode_sim(par,  # dictionary with model parameters
            ...
            num_circuit_genes, num_circuit_miscs, circuit_name2pos, sgpv,
            # dictionaries with circuit gene and miscellaneous specie names, species name to vector position decoder, relevant synthetic gene parameters in np.array form
            tf, ts, rtol, atol  # simulation parameters: time frame, when to save the system's state, relative and absolute tolerances
            ):
    # define the ODE term
    ode_for_integration = lambda t, x, args: ode_with_circuit(t, x, args)
    ...
    args = (
        ...
        sgpv  # relevant synthetic gene parameters in np.array form
    )

    # solve the ODE
    sol = scipy.integrate.solve_ivp(fun=ode_for_integration,    # ODE function
                                y0=x0,  # initial condition
                                t_span=tf,  # simulation time frame
                                t_eval=ts,  # recording times
                                args=(args,),  # arguments of the ODE function
                                method='LSODA', rtol=rtol, atol=atol  # integration method and tolerances
                                   )
...
def ode(t, x, circuit_ode, args):
    ...
    # resource competition denominator
    m_het_div_k_het = np.sum(x[8:8 + num_circuit_genes] / k_het)  # heterologous protein synthesis flux
    sum_mk_not_q = m_a / k_a + m_r / k_r + m_het_div_k_het
    mq_div_kq = (par['phi_q'] * (1 - prodeflux_div_eR) * sum_mk_not_q -
                 par['phi_q'] * prodeflux_div_eR) / (1 - par['phi_q'] * (1 - prodeflux_div_eR))
    D = (1 + mq_div_kq + sum_mk_not_q)  # resource competition denominator
...
def tauleap_sim(par,  # dictionary with model parameters
                ...
                sgpv, # relevant synthetic gene parameters in jax.array form
                ...
                mRNA_count_scales, S, circuit_synpos2genename, seed, # parameter vectors for efficient simulation and reaction stoichiometry info - determined by tauleap_sim_prep!!
                avg_dynamics = False # true if considering the deterministic cas with average dynamics of random variables
                ):
    # initialise random number generator
    np.random.seed(seed)

    # find how many Euler deterministic integration steps in one tau-leap step
    ...
    # initialise time record array
    ts=np.repeat(np.arange(tf[0],tf[1],tau_savetimestep),
                 repeats=num_trajs, axis=0)
    # initialise EMPTY state record array
    xs=np.zeros((num_trajs,len(ts),len(x0)))

    # define the ODE term
    ode_for_integration = lambda t, x, args: tauleap_ode(t, x, args)

    # simulate
    for traj_cntr in range(0,num_trajs):
        # set current time and state
        t_curr = tf[0]
        x_curr = x0
        xs[traj_cntr,0,:]=x_curr
        record_cntr = 1  # index of next point to be recorded in the trajectory
        not_all_records_obtained=True
        while (not_all_records_obtained):
            # get next simulated time point
            t_next = t_curr+tau

            # find stochastic changes in molecule counts
            stoch_update = tauleap_update_stochastically(t_curr, x_curr, tau, args, circuit_v, avg_dynamics)
            # find deterministic changes in molecule counts
            # det_update = tauleap_update_deterministically(t_curr, x_curr, tau, ode_steps_in_tau, args)
            odesol = scipy.integrate.solve_ivp(fun=tauleap_ode,    # ODE function
                                y0=x_curr,  # initial condition
                                t_span=(t_curr,t_next),  # simulation time frame
                                t_eval=[t_next],  # recording times
                                args=(args,),  # arguments of the ODE function
                                method='RK45', rtol=1e-6, atol=1e-6)  # integration method and tolerances
            det_update = odesol.y[:,0]-x_curr
            # find next x
            x_next = x_curr + det_update + stoch_update
            # make sure x has no negative entries
            x_next[x_next<0]=0

            # record the state if it's time to do so
            if(t_next>=ts[record_cntr]):
                xs[traj_cntr,record_cntr,:]=x_next
                record_cntr+=1
                not_all_records_obtained=(record_cntr<len(ts))
...
def tauleap_update_deterministically(t, x, tau, ode_steps_in_tau, args):
    ode_step_size = tau / ode_steps_in_tau

    # integrate the ODE
    x_new=x
    for i in range(0, ode_steps_in_tau):
        x_new=x + np.array(tauleap_ode(t + ode_step_size * i, x, args)) * ode_step_size
...
def tauleap_ode(t, x, args):
    ...
    return [
                         ...
                         # ribosomes inactivated by chloramphenicol
                         kcmh * B_cont - l * Bcm
                     ] + [0] * (2 * num_circuit_genes) + [0] * num_circuit_miscs # synthetic gene expression considered stochastically
...
def tauleap_update_stochastically(t, x, tau, args, circuit_v,
    ...
    v = circuit_v(t, x,  # time, cell state, external inputs
    ...
    # RETURN THE STOCHASTIC CHANGE IN MOLECULE COUNTS THAT EACH REACTION HAS OCCURRED
    # or average circuit dynamics
    if not(avg_dynamics):
        return np.matmul(S, np.random.poisson(lam=np.array(v) * tau))
    else:
        return np.matmul(S, np.array(v) * tau)
...
def tauleap_sim_prep(par,  # dictionary with model parameters
    ...
    mRNA_count_scales = np.array(mRNA_count_scales_np)
    ...
    return mRNA_count_scales, S, x0_tauleap, circuit_synpos2genename
...
def main():
    ...
    stoch_sim_duration = 0.1  # simulation time frame
    ...
    # simulate
    ts,xs = ode_sim(par,  # dictionary with model parameters
                  ...
                  cellmodel_auxil.synth_gene_params_vectorised(par, circuit_genes), # synthetic gene parameters for calculating k values
                  tf, np.arange(tf[0], tf[1], tau_savetimestep),  # time axis for saving the system's state
    ...
    mRNA_count_scales, S, x0_tauleap, circuit_synpos2genename = tauleap_sim_prep(par, len(circuit_genes),
                                                                                 len(circuit_miscs),
                                                                                 circuit_name2pos, det_steady_x)
    ts_stoch, xs_stoch = tauleap_sim(par,  # dictionary with model parameters
                                     ...
                                     num_trajs,  # number of stochastic trajectories to consider
                                     len(circuit_genes), len(circuit_miscs), circuit_name2pos,
                                     cellmodel_auxil.synth_gene_params_vectorised(par, circuit_genes),
                                     ...
                                     seed=225)  # starting random number generation key
    # concatenate the results with the deterministic simulation
    ts = np.concatenate((ts, np.array(ts_stoch)))
    xs_first = np.concatenate(
        (xs, np.array(xs_stoch[0])))  # getting the results from the first random number generator key in vmap
    xss = np.concatenate((xs * np.ones((num_trajs, 1, 1)), xs_stoch),
                         axis=1)  # getting the results from all vmapped trajectories
    # xss=xs * np.ones((num_trajs, 1, 1))
    ...
    nat_mrna_fig, nat_prot_fig, nat_trna_fig, h_fig = cellmodel_auxil.plot_native_concentrations_multiple(ts, xss, par,
                                                                                                 circuit_genes,
                                                                                              tspan=(tf[-1] - (tf_stoch[-1] - tf_stoch[0]), tf_stoch[-1]),
    ...
    het_mrna_fig, het_prot_fig, misc_fig = cellmodel_auxil.plot_circuit_concentrations_multiple(ts, xss, par, circuit_genes,
                                                                                       ...
                                                                                       circuit_styles, tspan=( tf[-1] - (tf_stoch[-1] - tf_stoch[0]), tf_stoch[-1]),
                                                                                                simtraj_alpha=0.1)  # plot simulation results
    F_fig = cellmodel_auxil.plot_circuit_regulation_multiple(ts, xss, par, circuit_F_calc, circuit_genes, circuit_miscs,
                                                    circuit_name2pos, circuit_styles, tspan=(tf[-1] - (tf_stoch[-1] - tf_stoch[0]), tf_stoch[-1]),
                                                    simtraj_alpha=0.1)  # plot simulation results
